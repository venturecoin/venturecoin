...
uint256 const hashGenesisBlock("0x00000013a60f0fdd8d1b638186cea317de485434e2e6426f0cf28dc779d59c53");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Venturecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Venturecoin Signed Message:\n";
...
{
    // Venturecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
	if (nHeight == 1) { return 3000000 * COIN; } // swap VENT x11
	int64 nSubsidy = 0.000000001 * COIN;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Venturecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Venturecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Blockchain.info Block #311647 2014-07-20 08:12:39";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0.00000001 * COIN;
        ...
        assert(block.hashMerkleRoot == uint256("0xcfbb7e865a7050f2f6fbec6c3da8dade6329008ed1ae328de2fdc6ea8f55e2aa"));
...
unsigned char pchMessageStart[4] = { 0xa1, 0xb9, 0xaa, 0x7f }; // Venturecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VenturecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VenturecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VenturecoinMiner(CWallet *pwallet)
{
    printf("VenturecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("venturecoin-miner");
    ...
    try { loop {
        ...
        printf("Running VenturecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VenturecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VenturecoinMiner, pwallet));
}
